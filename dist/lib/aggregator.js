...
exports.OutlierFilter = exports.PriceAggregator = void 0;
...
class OutlierFilter {
    remove(prices) {
        ...
        logger_1.logger.debug('Análise de outliers', {
        ...
        if (prices.length === 2) {
            ...
            if (diff > maxDiff) {
                logger_1.logger.debug('Diferença muito grande entre preços, usando menor', {
                    operation: 'price_search',
        ...
        if (sortedPrices.length >= 3) {
            const averageOfLowerHalf = sortedPrices
            ...
            logger_1.logger.debug('Análise de outliers - estatísticas', {
                operation: 'price_search',
            ...
            if (maxPriceRatio > 2.5) {
                filteredPrices = filteredPrices.filter(price => {
                    ...
                    if (isOutlier) {
                        logger_1.logger.debug('Preço removido como outlier', {
                            operation: 'price_search',
                ...
                if (filteredPrices.length === 0) {
                    logger_1.logger.debug('Todos os preços foram removidos no primeiro filtro, aplicando filtro mais permissivo', {
                        operation: 'price_search',
                    });
                    filteredPrices = sortedPrices.filter(price => {
                        ...
                        if (!keep) {
                            logger_1.logger.debug('Preço ainda muito alto', {
                                operation: 'price_search',
                                preco: price.toFixed(2),
                ...
                if (filteredPrices.length === 0) {
                    logger_1.logger.warn('Todos os preços foram removidos, usando média da metade inferior', {
                        operation: 'price_search',
                ...
                if (filteredPrices.length < sortedPrices.length) {
                    ...
                    logger_1.logger.debug('Outliers removidos', {
                        operation: 'price_search',
        ...
        logger_1.logger.debug('Análise IQR', {
            operation: 'price_search',
            q1: q1.toFixed(2),
            q3: q3.toFixed(2),
            iqr: iqr.toFixed(2),
        ...
        if (finalFiltered.length === 0) {
            ...
            logger_1.logger.warn('IQR removeu todos, usando mediana', {
                operation: 'price_search',
        ...
        if (totalRemoved > 0) {
            ...
            logger_1.logger.debug('Outliers removidos - resumo final', {
                operation: 'price_search',
...
exports.OutlierFilter = OutlierFilter;
